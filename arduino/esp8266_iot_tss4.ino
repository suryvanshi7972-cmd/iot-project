...
// Deployed Live Render Web Service URL:
const char* SERVER_BASE_URL = "https://iot-project-59i6.onrender.com"; 

// Optional Device Authentication Key
...
// Note for Pin D4:
// NodeMCU onboard LED is ACTIVE LOW (LOW = ON, HIGH = OFF).
// If using an external LED wired (D4 -> Resistor -> LED Anode -> GND), it is ACTIVE HIGH.
// Set to true for external LED (HIGH = ON), or false for active-low.
...
const unsigned long SENSOR_INTERVAL_MS = 10000; // Transmit DHT11 data every 10 seconds

unsigned long lastActuatorCheckTime = 0;
const unsigned long ACTUATOR_INTERVAL_MS = 3000; // Poll LED & LCD state every 3 seconds

// Cached states to prevent redundant redraws / flicker
...
void updateLcdDisplay(String line1, String line2) {
  // Truncate to maximum 16 characters per row
  if (line1.length() > 16) line1 = line1.substring(0, 16);
  if (line2.length() > 16) line2 = line2.substring(0, 16);

  // Redraw only when text changes to prevent display flickering
  ...
  Serial.println("[LCD] Screen Updated:");
  Serial.print("  Row 1: "); Serial.println(line1);
  Serial.print("  Row 2: "); Serial.println(line2);
}

// ---------------------- Helper: Setup HTTP / HTTPS Client -----
bool beginHttpConnection(HTTPClient &http, WiFiClientSecure &secureClient, WiFiClient &standardClient, const String &url) {
  http.setTimeout(15000); // 15-second timeout for cloud spin-up & network latency
  http.setFollowRedirects(HTTPC_STRICT_FOLLOW_REDIRECTS);

  if (url.startsWith("https://")) {
    secureClient.setInsecure(); // Bypass SSL thumbprint check for Render's rotating Let's Encrypt certificate
    secureClient.setTimeout(15000);
    return http.begin(secureClient, url);
  } else {
    standardClient.setTimeout(15000);
...
void sendSensorData(float temperature, float humidity) {
  ...
  if (!beginHttpConnection(http, secureClient, standardClient, url)) {
...
void syncActuators() {
  ...
  if (httpCode == HTTP_CODE_OK) {
    ...
    // Fallback to separate endpoints if unified sync encounters an issue
    http.end();

    // Check LED Endpoint
    String ledUrl = String(SERVER_BASE_URL) + "/api/led";
    if (beginHttpConnection(http, secureClient, standardClient, ledUrl)) {
    ...
    // Check LCD Endpoint
    String lcdUrl = String(SERVER_BASE_URL) + "/api/lcd";
    if (beginHttpConnection(http, secureClient, standardClient, lcdUrl)) {
      if (http.GET() == HTTP_CODE_OK) {
...
void setup() {
  ...
  // Initialize I2C Bus for LCD on D1 (SCL) and D2 (SDA)
...
void loop() {
  ...
  // 1. Task: Read DHT11 and transmit every 10 seconds
